...
const Signup = () => {
    useTitle('Sign Up')
    const { createAccount, updateNameAndPhoto, GoogleLogin } = useContext(AuthContext);
    ...
    const handleSignup = event => {
        ...
        createAccount(email, password)
            .then((result) => {
                ...
                updateNameAndPhoto(name, imageURL)
                    .then(() => {
                        form.reset()
                    }).catch((error) => {
                        console.error(error);
            ...
            className="py-5 flex flex-col items-center justify-center bg-gray-100"> <div className="flex flex-col bg-white shadow-md px-4 sm:px-6 md:px-8 lg:px-10 py-8 rounded-3xl w-50 max-w-md">
                ...
                <div className="mt-10">
                    <form onSubmit={handleSignup}>
                        ...
                        <div className="flex flex-col mb-5">
                            <label htmlFor="imageURL" className="mb-1 text-xs text-start tracking-wide text-gray-600">ImageURL:</label>
                            <div className="">
                                <div className=" inline-flex items-center justify-center absolute left-0 top-0 h-full w-10 text-gray-400 " >
                                    <i className="fas fa-at text-blue-500"></i>
                                </div>

                                <input id="imageURL" type="text" name="imageURL" className=" text-sm placeholder-gray-500 pl-3 pr-4 rounded-2xl border border-gray-400 w-full py-2 focus:outline-none focus:border-blue-400 " placeholder="ImageURL" />
                            </div>
                        </div>
                        <div className="flex flex-col mb-5">
